...
*/
@Service
public class IRPanUserServiceImpl extends ServiceImpl<RPanUserMapper, RPanUser>
    implements IRPanUserService {


    @Autowired
    private IRPanUserFileService irPanUserFileService;

    @Autowired
    private CacheManager cacheManager;

    @Override
    public R register(UserRegisterContext userRegisterContext) {
        //存入数据库
        doRegister(userRegisterContext);
        //创建根目录
        createRootFolder(userRegisterContext);
        //返回加密后的用户id
        return R.data(IdUtil.encrypt(userRegisterContext.getEntity().getUserId()));
    }

    @Override
    public R login(UserLoginContext userLoginContext) {
        //返回 token
        ...
        return R.data(userLoginContext.getAccessToken());
    }
    ...
    private RPanUserFile getUserRootFileInfo(Long userId) {
        return irPanUserFileService.getUserRootFile(userId);
    }
    ...
    private void createRootFolder(UserRegisterContext userRegisterContext) {
        ...
        irPanUserFileService.createFolder(createFileContext);
    }

    private void doRegister(UserRegisterContext userRegisterContext) {
        RPanUser user=new RPanUser();
